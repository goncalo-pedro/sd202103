...
class ServerControlSession(Thread):
    ...
    def run(self):
        ...
        with sockets.Socket.create_server_socket(self._shared_state.port + 1, game.ACCEPT_TIMEOUT) as control_socket:
    ...
    def dispatch_request(self) -> bool:
        ...
        last_request = False
        if request_type == game.LIST_CLIENTS:
            self._list_clients()
        elif request_type == game.BYE_OP:
            last_request = True
        elif request_type == game.STOP_SERVER_OP:
            last_request = True
